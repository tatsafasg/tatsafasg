...
local library = {
	...
	gui_parent = (function()
		local _, x, c = pcall(pcall, gethui or get_hidden_ui)
		if x and c then
			return c
		end
		x, c = pcall(function()
...
do
	...
	local nfo = TweenInfo.new(0.5, library.configuration.easingStyle, library.configuration.easingDirection)
	function updatecolors()
		local stacktime = osclock()
		lastcolor = stacktime
		delay(0.01, function()
			local dotween = lastcolor == stacktime
			if library.objects and (#library.objects > 0 or next(library.objects)) then
				for _, data in next, colored do
					local x, e = nil, nil
					if lastcolor == stacktime then
						x, e = pcall(function()
							local cclr = colors[data[3]]
							local darkness = data[4]
							tweenService:Create(data[1], nfo, {
...
function library:CreateWindow(options, ...)
		...
		function tabFunctions:CreateSection(options, ...)
			...
			function sectionFunctions:AddSearchBox(options, ...)
				...
				local Set = (multiselect and function(t, dat)
					...
					local lastv = library.flags[flagName]
					if not lastv or selectedOption ~= lastv then
						if lastv and type(lastv) == "table" then
							selectedOption = library.flags[flagName]
						else
							library.flags[flagName] = selectedOption
						end
						warn("Attempting to use new table for", flagName, " Please use :Set(), as setting it through flags table may cause errors\n", debug.traceback())
						lastv = library.flags[flagName]
					end
					local cloned = {unpack(selectedOption)}
					if not dat then
				...
				local function update()
					...
					local sstr = (passed_multiselect == "string" and multiselect) or (not multiselect and library.flags[flagName] and tostring(library.flags[flagName])) or (not multiselect and selectedOption and tostring(selectedOption)) or blankstring or "Nothing"
					if dropdownSelection.Text ~= sstr then
						dropdownSelection.Text = sstr
					end
					dropdownHeadline.Text = (dropdownName and tostring(dropdownName)) or "???"
				...
				local objectdata = {
					...
					RawSet = ((multiselect and function(t, dat)
						...
						if not lastv or selectedOption ~= lastv then
							...
							warn("Attempting to use new table for", flagName, " Please use :Set(), as setting through flags table may cause errors\n", debug.traceback())
							lastv = library.flags[flagName]
						end
						local cloned = {unpack(selectedOption)}
						if not dat then
			...
			function sectionFunctions:AddDropdown(options, ...)
				...
				local function AddOptions(optionsTable)
					if options.Sort then
						...
						elseif dosort ~= 1 and dosort ~= true then
							warn("Potential mistake for passed Sort argument:", dosort, "\n", debug.traceback())
						end
						if not didstuff then
							table.sort(optionsTable, library.defaultSort)
						end
				...
				local function update()
					dropdownName, callback = options.Name or dropdownName, options.Callback
					local sstr = (passed_multiselect == "string" and multiselect) or (library.flags[flagName] and tostring(library.flags[flagName])) or (selectedOption and tostring(selectedOption)) or blankstring or "nil"
					if dropdownSelection.Text ~= sstr then
						dropdownSelection.Text = sstr
					end
					dropdownHeadline.Text = (dropdownName and tostring(dropdownName)) or "???"
				...
				library.signals[1 + #library.signals] = colorPickerButton.MouseButton1Click:Connect(function()
					if submenuOpen == colorPicker or submenuOpen == nil then
						...
						if colorPickerEnabled then
							...
							print(library.flags[flagName]:ToHSV())
							print(colorH, colorS, colorV)
	...
	function windowFunctions:CreateDesigner(options, ...)
		...
		for _, v in next, {{"Main", "main"}, {"Background", "background"}, {"Outer Border", "outerBorder"}, {"Inner Border", "innerBorder"}, {"Top Gradient", "topGradient"}, {"Bottom Gradient", "bottomGradient"}, {"Section Background", "sectionBackground"}, {"Section", "section"}, {"Element Text", "elementText"}, {"Other Element Text", "otherElementText"}, {"Tab Text", "tabText"}, {"Element Border", "elementBorder"}, {"Selected Option", "selectedOption"}, {"Unselected Option", "unselectedOption"}, {"Hovered Option Top", "hoveredOptionTop"}, {"Unhovered Option Top", "unhoveredOptionTop"}, {"Hovered Option Bottom", "hoveredOptionBottom"}, {"Unhovered Option Bottom", "unhoveredOptionBottom"}} do
			...
			local cflag = "{"__Designer.Colors.topGradient":"000000","__Designer.Settings.ShowHideKey":"Enum.KeyCode.RightControl","__Designer.Colors.section":"B0AFB0","__Designer.Colors.hoveredOptionBottom":"2D2D2D","__Designer.Background.ImageAssetID":"rbxassetid://9681970193","__Designer.Colors.innerBorder":"000000","__Designer.Colors.unselectedOption":"282828","__Designer.Files.WorkspaceFile":"Maru Hub Premium","__Designer.Colors.unhoveredOptionTop":"323232","__Designer.Colors.outerBorder":"213052","__Designer.Background.ImageColor":"FFFFFF","__Designer.Colors.tabText":"B9B9B9","__Designer.Colors.elementBorder":"141414","__Designer.Background.ImageTransparency":65,"__Designer.Colors.sectionBackground":"0D0D0D","__Designer.Colors.selectedOption":"110A0A","__Designer.Colors.bottomGradient":"000000","__Designer.Colors.background":"090909","__Designer.Colors.hoveredOptionTop":"414141","__Designer.Colors.elementText":"939193","__Designer.Colors.main":"02B4D4","__Designer.Colors.otherElementText":"817F81","__Designer.Colors.unhoveredOptionBottom":"232323","__Designer.Background.UseBackgroundImage":true}" .. codename
		...
		local daaata = {{"AddTextbox", "__Designer.Textbox.ImageAssetID", backgroundsection, {
			Name = "Image Asset ID",
			Placeholder = "rbxassetid://9681970193",
			Flag = "__Designer.Background.ImageAssetID",
			Value = "rbxassetid://9681970193",
			...
			Name = "Workspace Name",--Maru Hub Premium
